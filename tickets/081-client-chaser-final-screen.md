...
- **What the screen shows** (`client/src/screens/ChaserFinalScreen.vue`, routed in `App.vue` for `currentScreen == "chaserFinal"`):
  - **Chaser view**: their question + answer input, their own score ticking up, the **team's target score** ("You're 3 behind the team"), and the 2-min countdown. **No buzz anywhere** — the prompt is shown and the Chaser types, exactly like the cash builder. The target is the live synced `teamScore` — a steal while the Chaser sits at 0 raises it (079's rule), so give the target number a small pop/flash when it changes. Win moment: when `chaserScore >= teamScore` the screen shows the caught banner (winner "chaser" from the `endGame` broadcast already delivered).
