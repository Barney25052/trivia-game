...
## Goal
Make the Chaser's 2-minute round a real race with win/lose resolution. Add `chaserScore` to state, a `submitFinalChaserAnswer` handler (correct → +1, immediate win when `chaserScore >= teamScore`; wrong → the team gets a steal chance), wire the steal window, and remove the placeholder self-report `finalChaserScore` handler (currently a button that instantly wins — `messageHandlers.ts:276-287`). **The Chaser answers directly, with no buzz-in** — the prompt is shown and they type, exactly like the cash builder; buzzing is only ever a team-side mechanic. **Win = reaching the target**: the game ends the moment `chaserScore >= teamScore` — a tie counts as reached, no further questions.
...
- Replace `finalChaserScore` (`messageHandlers.ts:276-287` + its `TriviaRoom.ts:330-333` registration) with `submitFinalChaserAnswer` (`{ questionId, answer }`), guarded: phase `ChaserFinal`, seat is the Chaser, shape valid, `questionId` matches the current Chaser question, once per question. **No buzz gate on this handler — the Chaser types directly.**
  ...
  - **Wrong** → steal: send the **team** seats a `finalSteal` message (`{ questionId, prompt, windowMs }` — the same prompt the Chaser just missed, never the answer), start a `stealWindowMs` (20s) timer. **No buzz** — any non-Chaser may send `submitFinalStealAnswer` (`{ questionId, answer }`), and the **first submitted answer resolves the steal, even if it's wrong** (the window closes immediately; subsequent submissions are rejected as stale). Correct → **push-back while above 0, raise the target when at 0** (TO_REVIEW #18, decided 2026-09-12): `chaserScore > 0` → `chaserScore -= 1`; `chaserScore === 0` → `teamScore += 1` (the steal always moves the goalposts — a Chaser at 0 can't be pushed below, so the team's target goes up one instead). Either way the Chaser advances to the next question; wrong, or nobody submitted before the window expired → no effect, Chaser advances. Only one steal outcome per question.
...
- New `server/test/finalRound.test.ts` (extend 077/078's file):
  ...
  - Chaser wrong answer → team receives `finalSteal` with the prompt, `windowMs` = 20000, and **no answer**; the **first `submitFinalStealAnswer` from any non-Chaser resolves the steal** — a correct steal at `chaserScore > 0` → `chaserScore - 1`; a correct steal at `chaserScore === 0` → `teamScore + 1` (the team's target climbs; the Chaser's "X behind" display updates via the synced `teamScore`); a wrong first answer → no effect (and the window is closed — a second answer is rejected). **No buzz involved in the steal.**
